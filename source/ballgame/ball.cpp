...
Ball::Initialise(Renderer& renderer)
{
	...
	int xCorner = GetRandom(0, 1);
	int yCorner = GetRandom(0, 1);
	if (xCorner)
	{
		m_position.x = static_cast<float>(GetRandom(EDGE_LIMIT, SCREEN_WIDTH / 4));
	}
	else
	{
		m_position.x = static_cast<float>(GetRandom(SCREEN_WIDTH - (SCREEN_WIDTH / 4), SCREEN_WIDTH - EDGE_LIMIT));
	}
	
	if (yCorner)
	{
		m_position.y = static_cast<float>(GetRandom(EDGE_LIMIT, SCREEN_HEIGHT / 4));
	}
	else
	{
		m_position.y = static_cast<float>(GetRandom(SCREEN_HEIGHT - (SCREEN_HEIGHT / 4), SCREEN_HEIGHT - EDGE_LIMIT));
	}
...
void 
Ball::SetColour(unsigned char r, unsigned char g, unsigned char b)
{
	m_pSprite->SetRedTint(static_cast<float>(r) / 255);
	m_pSprite->SetGreenTint(static_cast<float>(g) / 255);
	m_pSprite->SetBlueTint(static_cast<float>(b) / 255);
}
...
Ball::RandomiseSize()
{
	float scale = static_cast<float>(GetRandom(2, 5)) / 10.f;
